...
def do_train(
    ...
    for epoch in range(arguments["epoch"], max_epoch + 1):
        ...
        for iteration, (batches, idx) in enumerate(data_loader):
            ...
            optimizer.zero_grad()
            contr_weight = cfg.MODEL.SMTAN.LOSS.CONTRASTIVE_WEIGHT
            consis_weight = cfg.MODEL.SMTAN.LOSS.CONSIS_WEIGHT
            exc_weight = cfg.MODEL.SMTAN.LOSS.EXC_WEIGHT
        ...
        if checkpoint_period != -1 and epoch % checkpoint_period == 0:
            checkpointer.save(f"{cfg.MODEL.SMTAN.FEAT2D.NAME}_model_{epoch}e", **arguments)
