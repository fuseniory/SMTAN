...
class ContrastiveLoss(object):
    def __init__(self, cfg, mask2d):
        self.mask2d = mask2d
        self.T_v = cfg.MODEL.SMTAN.LOSS.TAU_VIDEO  # 0.1
        self.T_s = cfg.MODEL.SMTAN.LOSS.TAU_SENT  # 0.1
        self.cri = torch.nn.CrossEntropyLoss()
        self.neg_iou = cfg.MODEL.SMTAN.LOSS.NEGATIVE_VIDEO_IOU  # 0.5
        self.top_k = cfg.MODEL.SMTAN.LOSS.NUM_POSTIVE_VIDEO_PROPOSAL  # 1
        self.sent_removal_iou = cfg.MODEL.SMTAN.LOSS.SENT_REMOVAL_IOU  # 0.5
        self.margin = cfg.MODEL.SMTAN.LOSS.MARGIN  # 30
        self.eps = 1e-6
    ...
    def __call__(self, feat2ds, sent_feats, iou2ds, phr_feats, phr_weights, gt_proposals):
        ...
        for i, (sent_feat, iou2d) in enumerate(zip(sent_feats, iou2ds)):  # each video in the batch
            ...
            iou1d = iou2d.masked_select(self.mask2d).reshape(sent_feat.size(0),
                                                             -1)  # num_sent x num_sparse_selected_proposal
            ...
            selected_feat = feat1d.index_select(dim=1, index=topk_index.reshape(-1)).reshape(C, -1,
                                                                                             self.top_k)  # C x num_sent x top_k
            ...
            vid_pos = torch.bmm(selected_feat,
                                sent_feat.unsqueeze(2)).reshape(-1,
                                                                self.top_k) - self.margin  # num_sent x top_k, bmm of (num_sent x top_k x C) and (num_sent x C x 1)
            vid_neg = torch.mm(selected_feat.view(-1, C),
                               sent_feat_cat_norm.t()).reshape(-1, self.top_k,
                                                               sum_num_sent)  # num_sent x topk x sum(num_sent), mm of (num_sent*top_k x C) and (C x sum(num_sent))
            ...
            iou_neg_mask = (
                        iou1d < self.neg_iou).float()  # only keep the low iou proposals as negative samples in the same video
            ...
            feat1d_other_video = feat1ds_norm.index_select(dim=0, index=torch.arange(
                ...
                torch.arange(B, device=feat2ds.device) != i])  # (B-1) x C x num_sparse_selected_proposal
            feat1d_other_video = feat1d_other_video.transpose(1, 0).reshape(C,
                                                                            -1)  # C x ((B-1) x num_sparse_selected_proposal)
            sent_neg_other_video = torch.mm(sent_feat,
                                            feat1d_other_video)  # num_sent x ((B-1) x num_sparse_selected_proposal)
        ...
        vid_neg = torch.cat(vid_neg_list, dim=0).permute(1, 0,
                                                         2)  # top_k x this_cat_to_be_sum(num_sent) x sum(num_sent)
        vid_neg = (
                              vid_neg - margin_mask) / self.T_v  # top_k x this_cat_to_be_sum(num_sent) (positive) x sum(num_sent) (negative)
        sent_mask += torch.diag(torch.ones(sum_num_sent, device=feat2ds.device)).float()
